function [W_estimate, dif_W] = gc_estimate_W_2d_1(MU_smoother, SIGMA_smoother, Yk, mu, W, L, incongruent_vec, iter, W_sim, update_coef_W)
        ...
        if updata_val==1
            ...
            switch ww
                case 1
                    ...
                    x = fminsearch(@(x)f_update_wm1(x, Wm2, Wm3, MU_smoother, SIGMA_smoother, Yk, mu, Um, incongruent_vec),x0);
...
% % %                     func_plot_wm1(Wm1, Wm2, Wm3, MU_smoother, SIGMA_smoother, Yk, mu, Um, x, m, iter, W_sim_ch(1))
                ...
                case 2
                    ...
                    % call find min function
                    x = fminsearch(@(x)f_update_wm2(x, Wm1, Wm3, MU_smoother, SIGMA_smoother, Yk, mu, Um, incongruent_vec),x0);
...
%                     end
                ...
                case 3
                    ...
                    % call find min function 
                    x = fminsearch(@(x)f_update_wm3(x, Wm1, Wm2, MU_smoother, SIGMA_smoother, Yk, mu, Um, incongruent_vec),x0);
...
function obj = f_update_wm1(x, Wm2, Wm3, MU_smoother, SIGMA_smoother, Yk, mu, Um, incongruent_vec)
...
for k=1: K
    ...
    % involving incongurent effect
    Wm_hat_k = [x Wm2*incongruent_vec(k)];
    Wm_hat_k = Wm_hat_k.';
    ...
    Exp_term = exp(-((Wm_hat_k.' * (mu_s-.5*sigma_s*Wm_hat_k))) - Wm_biased);
...
obj = obj + Wm_hat_k.'*mu_hat  +  K*Wm_biased;
% obj = -obj - Wm_hat.'*mu_hat  -  K*Wm_biased;
end

function obj = f_update_wm2(x, Wm1, Wm3, MU_smoother, SIGMA_smoother, Yk, mu, Um, incongruent_vec)
...
for k=1: K
    ...
    % involving incongurent effect
    Wm_hat_k = [Wm1 x*incongruent_vec(k) ];
    Wm_hat_k = Wm_hat_k.';
...
function obj = f_update_wm3(x, Wm1, Wm2, MU_smoother, SIGMA_smoother, Yk, mu, Um, incongruent_vec)
...
for k=1: K
    ...
    Wm_hat_k = [Wm1 Wm2*incongruent_vec(k)];
    Wm_hat_k = Wm_hat_k.';
    % p_k - Eq 2.a (UPDATE_L_W.docx)
    y_k = Yk(k, :).';
    p_k = y_k- mu;
    ...
    % estimate E[lambda_inv] - Eq 4.c (UPDATE_L_W.docx)
    Exp_term = exp(-((Wm_hat_k.' * (mu_s-.5*sigma_s*Wm_hat_k))) - x);
